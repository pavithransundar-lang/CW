...
export const SettingsModal: React.FC<Props> = ({ isOpen, onClose, settings, onSave, onReset }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 backdrop-blur-sm">
      <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl transform transition-all">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {activeTab === 'general' && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Max Class Earnings</label>
                  <input 
                    type="number" 
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                {formData.shopItems.map((item, idx) => (
                  <div key={item.id || idx} className="flex items-end gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <div className="flex-1">
                      ...
                      <input type="text" value={item.label} onChange={(e) => {
                        ...
                        setFormData({...formData, shopItems: newItems});
                      }} className="w-full border-gray-300 rounded-md border p-1.5 text-sm focus:ring-emerald-500 focus:border-emerald-500" />
                    </div>
                    <div className="w-24">
                      <label className="text-xs text-gray-500 font-medium ml-1">Cost ({formData.currencySymbol})</label>
                    ...
                    <div className="w-20">
                      <label className="text-xs text-gray-500 font-medium ml-1">Mins</label>
              ...
              <button 
                ...
                className="w-full py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-emerald-500 hover:text-emerald-600 hover:bg-emerald-50 transition-all text-sm font-medium flex items-center justify-center gap-2"
              >
          ...
          {activeTab === 'goals' && (
            <div className="space-y-3">
              {formData.goals.map((goal, idx) => (
                <div key={idx} className="flex gap-2 items-center">
                   <div className="bg-emerald-100 text-emerald-600 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0">
                     {idx + 1}
                   </div>
                   <input 
                    type="text" 
                    ...
                    className="flex-1 border-gray-300 rounded-lg border p-2 text-sm focus:ring-emerald-500 focus:border-emerald-500" 
                  />
                  <button onClick={() => {
              ...
              <button onClick={() => setFormData({...formData, goals: [...formData.goals, "New Goal"]})} className="text-sm text-emerald-600 font-semibold hover:text-emerald-700 flex items-center gap-1 mt-2">
                <span>+</span> Add Goal
              </button>
            </div>
          )}
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
          ...
          }} className="text-red-600 text-sm hover:text-red-800 hover:underline px-2">Reset All Data</button>
          
          <div className="flex gap-3">
            <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-200 rounded-lg transition">Cancel</button>
